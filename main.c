...
int main(void)
{
	...
	TFT_init();
	TFT_led(1);
    /*меняем ориентацию под домофон (3)*/
	TFT_sendCMD(0x36);
	TFT_sendDATA(0xE8);
	...
	jpeg_buffer.buffer = signal_jpg;
	jpeg_buffer.length = signal_jpg_size;
	...
	if (!status) {
		...
		if (pImage) {
			drawRGBBitmap(pImage, 200-image_info.m_width, 320-image_info.m_height, image_info.m_width, image_info.m_height, image_info.m_comps);
			free(pImage);
		}
	}
